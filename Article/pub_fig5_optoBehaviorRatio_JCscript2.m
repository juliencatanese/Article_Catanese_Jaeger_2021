...
for of=1:NSess-1
    ...
    % DIFFERENCE R-L
    CRvsCL_diff=[CRvsCL_diff (trial.Nb_correct_R+1)-(trial.Nb_correct_L+1)];
    CRovsCLo_diff =[CRovsCLo_diff (trial.Nb_correct_R_opto+1)-(trial.Nb_correct_L_opto+1)];
    
    impRvsimpL_diff = [impRvsimpL_diff (Nimpulse_CR+1)-(Nimpulse_CL+1)]
    impRovsimpLo_diff = [impRovsimpLo_diff (Nimpulse_CR_opto+1)-(Nimpulse_CL_opto+1)]
    
    erPRvserPL_diff = [erPRvserPL_diff (NerrSide_PR+1)-(NerrSide_PL+1)]
    erPRovserPLo_diff = [erPRovserPLo_diff (NerrSide_PR_opto+1)-(NerrSide_PL_opto+1)]
    
    omiRvsomiL_diff = [omiRvsomiL_diff (Nnolick_R+1)-(Nnolick_L+1)]
    omiRovsomiLo_diff = [omiRovsomiLo_diff (Nnolick_R_opto+1)-(Nnolick_L_opto+1)] 
