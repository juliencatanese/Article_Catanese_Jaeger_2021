...
X1= Tfig1_VMopto.AP(logical(Tfig1_VMopto.VMVL & Tfig3_RvL.dzRL_exct));
Y1= Tfig1_VMopto.ML(logical(Tfig1_VMopto.VMVL & Tfig3_RvL.dzRL_exct));
Z1= Tfig1_VMopto.DV(logical(Tfig1_VMopto.VMVL & Tfig3_RvL.dzRL_exct));

X2= Tfig1_VMopto.AP(logical(Tfig1_VMopto.VMVL & Tfig3_RvL.dzRL_inib));
Y2= Tfig1_VMopto.ML(logical(Tfig1_VMopto.VMVL & Tfig3_RvL.dzRL_inib));
Z2= Tfig1_VMopto.DV(logical(Tfig1_VMopto.VMVL & Tfig3_RvL.dzRL_inib));
...
ylabel('ML')
zlabel('DV')
legend ('contra','ipsi', 'VM', 'VAL')

saveas(gcf, [SaveFolder '\S4A.fig'])
saveas(gcf, [SaveFolder '\S4A.png'])
saveas(gcf, [SaveFolder '\S4A.emf'])

% Distrib and stats 
disp('AP')
[Px1 Hx1] = ranksum(X1,X2); if Hx1; Px1, end  % Px1 = 0.0364
disp('ML')
[Py1 Hy1] = ranksum(Y1,Y2); if Hy1; Py1, end
disp('DV')
...
subplot(3,1,1)
hold on
histogram(X1,'Normalization','count'), histogram(X2,'Normalization','count')
xlabel('AP')
legend('contra cells','ipsi cells')
subplot(3,1,2)
hold on
histogram(Y1,'Normalization','count'), histogram(Y2,'Normalization','count')
xlabel('ML')
legend('contra cells','ipsi cells')
subplot(3,1,3)
hold on
histogram(Z1,'Normalization','count'), histogram(Z2,'Normalization','count')
xlabel('DV')
legend('contra cells','ipsi cells')

saveas(gcf, [SaveFolder '\S4B.fig'])
saveas(gcf, [SaveFolder '\S4B.png'])
saveas(gcf, [SaveFolder '\S4B.emf'])
...
subplot(3,1,3)
hold on
BMIN = max([min(Z1) min(Z2) ]); 
BMAX = min([max(Z1) max(Z2) ]); 
Hy1 =histogram(Z1,'Normalization','count', 'BinWidth',0.1,'BinLimits',[BMIN,BMAX]); 
Hy2 =histogram(Z2,'Normalization','count', 'BinWidth',0.1,'BinLimits',[BMIN,BMAX]);
xlabel('DV')
legend('Task+ cells','Task- cells')
