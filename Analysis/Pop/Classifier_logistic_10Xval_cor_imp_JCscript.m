...
tr_type_comp1 = {'cor', 'imp'};
tr_type_comp2 = {'cor', 'oNO'};
tr_type_comp3 = {'oNO', 'imp'};

Comp_list=[{tr_type_comp1} {tr_type_comp2} {tr_type_comp3}];
trial_type_list =[];

for Trtyp=1:3
    
    trial_type_list = Comp_list{Trtyp}
    ...
    for Ev=1:3
        ...
        for ncluf = 1:max(size(clust_file)) %  channel loop
            ...
            for CLUST= 1:Nclust %Cluster loop within each channel
                ...
                for ii=1:NbTtrialType
                    if trial_type_list{ii} == 'cor'
                        trigtimes = trigtimes_cor ;
                    elseif trial_type_list{ii} == 'imp'
                        trigtimes = trigtimes_imp ; %disp('Correct Left trials');
                    elseif trial_type_list{ii} == 'oNO'
                        trigtimes = trigtimes_oNO ; %disp('Correct Right trials');
                    elseif trial_type_list{ii} == 'opt'
                        trigtimes = trigtimes_opt ; %disp('Correct Right trials');
                    end
        ...
        for ll=1:max(size(learner))
            ...
            elseif Trtyp==2 % cor vs oNO
                %                 col = [0.5 0.2 0.7]
                plot( [1:1:ncell] , M, 'Color','c', 'LineWidth', 2)
                M2=M; save(['ClassM2_' psth_center_evtID '_'  trial_type_list{1} '_' trial_type_list{2}]','M2')
                
            elseif Trtyp==3 % oNO vs imp
                col = [0.4 0.4 0.4]
                plot( [1:1:ncell] , M, 'Color',col, 'LineWidth', 2)
                M3=M; save(['ClassM3_' psth_center_evtID '_'  trial_type_list{1} '_' trial_type_list{2}]','M3')
        ...
        ylim([0 1])
        xlabel('#Neurons', 'FontSize', 10)
        ylabel('Proba errors', 'FontSize', 10)
        title(psth_center_evtID, 'FontWeight','bold' ,'FontSize', 12)        
    end
end
%%
legend(['chance level  (Mouse: ' Mouse ' ' Day ')'], ' cor-imp Mean LogClass10X Nrepet20', 'cor-oNO Mean LogClass10X Nrepet20' , 'imp-oNO Mean LogClass10X Nrepet20' )
