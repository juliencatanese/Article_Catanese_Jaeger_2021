...
load([ChanID{1} '_raw.mat']) % data = microV and sr = sampling rate
...
[B,A] = butter(3,[15/Nyquist 35/Nyquist]); % band pass filter (pass between 5-50Hz)
...
for tr =1:max(size(tr_ID_Resp_good))
    ...
    %     Plot Normnalized Spectro for each trial
    nP=[];
    for i=1 : max(size(T))
        totP = sum(P(:,i));
        nP(:,i) = P(:,i) ./ totP;
    ...
    mkdir('Spectrogram')
    saveas(gcf,['.\Spectrogram\' ChanID{1}  '_Spectro_trial#' num2str(tr)], 'png')
end
%% plot AVERAGED Normalized Spectrogram (Normalized to the total power at each time bin)
figure,
...
xlabel('Time (sec)')
%%
saveas(gcf,['.\Spectrogram\' ChanID{1}  '_Spectro_Mean_Norm_Centered'], 'png')
% saveas(gcf,['.\' ChanID{1}  '_Spectro_Mean_Norm_Centered'], 'png')
