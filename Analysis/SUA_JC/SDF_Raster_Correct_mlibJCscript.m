...
for ncluf = 1:max(size(clust_file)) %  channel loop
    ...
    for CLUST= 1:Nclust %Cluster loop within each channel
        ...
        for ii=1:NbTtrialType
            ...
            for i = 1:numel(trialspx)
                ...
                elseif ii==2
                    ...
                    title([ 'raster center on ' psth_trig_evt ' ' psth_trial_type{ii} 'trials'])
                    legend({['Opto ' psth_trial_type{ii}]}, 'FontSize',11,'FontWeight', 'normal')
        ...
        saveas(gcf, ['D:\JC_Figures\SDFopto\' info.info_notes.MouseID '_'  clust_file(ncluf).name(7:11) '_'  clust_file(ncluf).name(7:11) ' clust#' num2str(CLUST) '_SDF_' tagsave 'center_' psth_trig_evt ], 'png')
