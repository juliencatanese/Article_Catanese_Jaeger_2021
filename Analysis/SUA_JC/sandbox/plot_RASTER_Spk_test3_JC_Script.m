...
% define Block size and Number for future figure
...
clust_file = dir('times_S*Ch*_raw*.mat')
for ncluf = 1: max(size(clust_file)) %  channel loop 
    ...
    for CLUST= 1:max(cluster_class(:,1)) %Cluster loop within each channel
        ...
        for Nb = 1:Nblock-1
            ...
            %     xlim([0,13])
            saveas(gcf,[info.info_notes.MouseID '_'  info.info_notes.Day(1:end-2) '_' clust_file(ncluf).name(7:13) '_CLUST#' num2str(CLUST) '_block#' zz num2str(Nb) ],'tif')
            close gcf 
        end
    end
end
