    ...
    for i=1:2:size(varargin,2)
        switch varargin{i}
            ...
            case 'chart'
                chart = varargin{i+1};
            otherwise
                disp('ATTENTION mspth error: unknown argument'); forcestop;
        end
    end
else
end
...
for i = 1:numel(trigtimes) % for each trial
    clear spikes
...
if fr==1
    psth (:,2) = (1/binsz)*1000*psth(:,2)/numel(trigtimes); % divide by number of trials
end
%% remove time base
if tb==0
    psth(:,1) = [];
