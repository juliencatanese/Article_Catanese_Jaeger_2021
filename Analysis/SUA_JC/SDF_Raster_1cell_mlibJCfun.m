...
Ntrials= trial.Ntrial; MouseID 
% if MouseID(end-1:end) == '11'
%     Ntrials = 150
%     trial.idx_correct_L=  trial.idx_correct_L(1:Ntrials)
%     trial.idx_correct_R= trial.idx_correct_R(1:Ntrials)
% end
...
%         end
    ...
    elseif psth_trig_evt=='Licks'
        lick_tr=  evt_lick(idx_trial_start(tr):idx_trial_start(tr)+2.25*sr);
...
for ii=1:NbTtrialType
    ...
    if MouseID == 'vgat12'; trigtimes=trigtimes(1:10); end
%     if MouseID == 'vgat14'; trigtimes=trigtimes(1:17); end
    %% Compute SDF
    %     pre=1500; %ms
    %     post=1500;%ms
...
for ptt=1:NbTtrialType
    tag_trtype = [tag_trtype '_' psth_trial_type{ptt}];
end
% xlim([-1500 1500])
xlim([-pre+250 post-250])

saveas(gcf, ['fig2print_'  ChanID '_clust#' num2str(CLUST) '_SDF_' tag_trtype '_' psth_trig_evt ], 'jpg')
