function []=Artifact_removal_1ChanRef_JC_fun(ref_Chan)
% function Artifact_removal_1ChanRef_JC_fun(ref_Chan)
% INPUT:
% ref_chan = 'S1Ch2' ==> ref manually choosen.
% ref_chan =  '' (empty) ==>  ref automatically choosen: chan with less #Spk detected (before clustering)
% Written by Julien Catanese
% Last Updated: 10/08/2018

clearvars -except ref_Chan
list_nspk= [];
CurrentDir = pwd;

if nargin == 0;
    spikes_ChID = dir([CurrentDir '\*_raw_spikes.mat']);
    if ~isempty(spikes_ChID)
        ...
        disp('ATTENTION NO files named *_raw_Spikes.mat, NEED to RUN AUTOCLUS')
    end
elseif nargin==1
    Rchan=dir([ref_Chan '_raw.mat'])
    refID=Rchan.name
    save('RefChan', 'refID');
elseif nargin>1
...
raw_ChID = dir([CurrentDir '\S*Ch*_raw.mat']);
for nrc= 1:max(size(raw_ChID))
    ...
    saveas(gcf, ['fig2print_' ShChID(1:5) '_Artifact1b_removal_trh'],'jpg')
