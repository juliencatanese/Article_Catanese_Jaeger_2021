...
% Generate structure with 8 field, to create a 8 column boolean table, with each row correspond to 1 trial in order of appearence during the task. 
...
trig_end = find(diff(evt_trial)<0); % NOT RESTRICTed TO TASK EPOCH 
trig_st = find(diff(evt_trial)>0); % NOT RESTRICTed TO TASK EPOCH 
