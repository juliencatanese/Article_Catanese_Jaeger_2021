function rhd_struct_output = read_Intan_RHD2000_file_JC(Folder,File)
...
clearvars -EXCEPT Folder File Data_allrhd rhd_struct_output
...
rhd_struct_output = struct('filename',filename);

rhd_struct_output.notes = (notes);
rhd_struct_output.frequency_parameters = (frequency_parameters);

if (num_amplifier_channels > 0)
    rhd_struct_output.amplifier_channels = (amplifier_channels);
    if (data_present)
        rhd_struct_output.amplifier_data = (amplifier_data);
        rhd_struct_output.t_amplifier = (t_amplifier);
    end
    rhd_struct_output.spike_triggers = (spike_triggers);
end
if (num_aux_input_channels > 0)
    rhd_struct_output.aux_input_channels = (aux_input_channels);
    if (data_present)
        rhd_struct_output.aux_input_data = (aux_input_data);
        rhd_struct_output.t_aux_input = (t_aux_input);
    end
end
if (num_supply_voltage_channels > 0)
    rhd_struct_output.supply_voltage_channels = (supply_voltage_channels);
    if (data_present)
        rhd_struct_output.supply_voltage_data = (supply_voltage_data);
        rhd_struct_output.t_supply_voltage = (t_supply_voltage);
    end
end
if (num_board_adc_channels > 0)
    rhd_struct_output.board_adc_channels = (board_adc_channels);
    if (data_present)
        rhd_struct_output.board_adc_data = (board_adc_data);
        rhd_struct_output.t_board_adc = (t_board_adc);
    end
end
if (num_board_dig_in_channels > 0)
    rhd_struct_output.board_dig_in_channels = (board_dig_in_channels);
    if (data_present)
        rhd_struct_output.board_dig_in_data = (board_dig_in_data);
        rhd_struct_output.t_dig = (t_dig);
    end
end
if (num_board_dig_out_channels > 0)
    rhd_struct_output.board_dig_out_channels = (board_dig_out_channels);
    if (data_present)
        rhd_struct_output.board_dig_out_data = (board_dig_out_data);
        rhd_struct_output.t_dig = (t_dig);
    end
end
if (num_temp_sensor_channels > 0)
    if (data_present)
        rhd_struct_output.temp_sensor_data = (temp_sensor_data);
        rhd_struct_output.t_temp_sensor = (t_temp_sensor);
    end
end
